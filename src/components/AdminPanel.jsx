...
export default function AdminPanel({ session }) {
  ...
  // 1. Deklarasikan fetchAdminData dengan useCallback agar stabil
  const fetchAdminData = useCallback(async () => {
  setLoading(true)
  const { data: profiles } = await supabase.from('profiles').select('*')
  const { data: todosData } = await supabase.from('todos').select('*')
  
  // 👈 Fetch 5 aktivitas terbaru dari tabel activities
  ...
  // 2. Jalankan fetch pertama kali saat komponen di-mount
  useEffect(() => {
  // Memindahkan eksekusi ke microtask queue agar tidak dianggap sinkron oleh ESLint
  queueMicrotask(() => {
    fetchAdminData()
  })
}, [fetchAdminData])
  ...
  // Deklarasikan email yang kebal terhadap perubahan peran
...
const toggleRole = async (userId, currentRole, targetEmail) => {
  ...
  // Handler untuk pencarian & filter sekaligus reset halaman ke 1
  ...
  // Helper format waktu relatif (misal: "2 menit lalu")
  ...
  // Tambahkan state ini bersama state lainnya
  const [onlineUsers, setOnlineUsers] = useState(new Set())

  // ✅ FIX: Presence Supabase Realtime yang Aman & Kebal Crash
  useEffect(() => {
    // Safety guard: Jika session belum ada, jangan jalankan realtime
    ...
    // 2. DAFTARKAN SEMUA EVENT LISTENER (.on) SEBELUM SUBSCRIBE!
    channel
      .on('presence', { event: 'sync' }, () => {
        try {
          ...
          // Loop untuk mengambil user_id asli dari objek presence
    ...
    // 3. BARU PANGGIL .subscribe() DI PALING AKHIR
    ...
    // 4. Clean up channel dengan benar
    return () => {
      channel.unsubscribe()
  ...
  const handleExportLog = () => {
    ...
    // Buat header dan baris CSV
    const headers = ['ID', 'Judul Aktivitas', 'Waktu Dibuat']
    const csvRows = [
      headers.join(','), // Row header
      ...recentActivities.map((act) =>
        [
          `"${act.id || ''}"`,
          `"${(act.title || '').replace(/"/g, '""')}"`, // escape quote
          `"${act.created_at || ''}"`,
        ].join(',')
      ),
    ]
  ...
  // 2. Fungsi Flush Cache & Fetch Ulang
  const handleFlushCache = async () => {
    setIsFlushing(true)
    try {
      // Hapus cache lokal browser jika ada data temporary yang tersimpan
      localStorage.clear()
      sessionStorage.clear()

      // Tarik ulang data segar dari server
  ...
  // Helper untuk mengelompokkan data tugas 7 hari terakhir
  const getWeeklyAnalytics = (todosList) => {
    const daysMap = ['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab']
    const today = new Date()
    
    // Inisialisasi struktur 7 hari terakhir
    ...
    // Hitung akumulasi tugas berdasarkan created_at
   ...
   // Mock Trend Analytics (SaaS Style)
      ...
      <div className="max-w-7xl mx-auto px-6 pt-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-6 border-b border-[#E4DFD3] dark:border-[#3A3733]">
          <div className="space-y-1">
            <span className="font-mono text-[10px] tracking-[0.2em] uppercase text-[#6B8874]">Panel administrasi</span>
          ...
          <div className="flex items-center gap-3">
            {/* Ubah tombol Sinkronkan Data di Welcome Section */}
            <button 
        ...
        {/* 2.  SIX STATS CARDS  WITH ONLINE USERS */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            { title: 'Total User', value: totalUsers, accent: '#3D5F49' },
            { title: 'User Online', value: onlineUsers.size, accent: '#10B981', isOnline: true }, // 👈 Kartu User Online
            ...
            <div key={idx} className="bg-white dark:bg-[#211F1C] border border-[#E4DFD3] dark:border-[#3A3733] border-l-2 p-4 rounded-md shadow-sm space-y-2" style={{ borderLeftColor: stat.accent }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white dark:bg-[#211F1C] border border-[#E4DFD3] dark:border-[#3A3733] p-6 rounded-md space-y-4 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-serif text-slate-900 dark:text-white">Analitik Tugas Harian</h3>
                <p className="text-xs text-slate-400">Tren pembuatan vs penyelesaian tugas 7 hari terakhir</p>
              </div>
              {/* Legenda Warna */}
          ...
          <div className="bg-white dark:bg-[#211F1C] border border-[#E4DFD3] dark:border-[#3A3733] p-6 rounded-md space-y-4 shadow-sm flex flex-col justify-between">
            <div>
              <h3 className="text-sm font-serif text-slate-900 dark:text-white">Distribusi Status Tugas</h3>
            ...
            <div className="flex justify-center gap-6 text-xs font-bold">
              <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-[#3D5F49]"></span> Aktif ({activeTodos})</div>
              <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-[#C99A2E]"></span> Selesai ({completedTodos})</div>
            </div>
          </div>
        </div>

        {/* 4. SECONDARY SECTION (Top Active Users, Activity, Health) */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Top Active Users Chart */}
          <div className="bg-white dark:bg-[#211F1C] border border-[#E4DFD3] dark:border-[#3A3733] p-6 rounded-md space-y-4">
            ...
            <div className="h-52">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topUsersChart} layout="vertical">
                  ...
                  <Tooltip contentStyle={{ backgroundColor: '#211F1C', borderRadius: '6px', fontSize: '11px' }} />
                  <Bar dataKey="Total" fill="#C99A2E" radius={[0, 8, 8, 0]} barSize={16} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-[#211F1C] border border-[#E4DFD3] dark:border-[#3A3733] p-6 rounded-md space-y-4">
            ...
            <div className="space-y-3 text-xs">
              {recentActivities.length === 0 ? (
                ...
                recentActivities.map((act) => (
                  <div key={act.id} className="flex items-start gap-3 p-2.5 rounded-md bg-[#F5F3ED] dark:bg-[#1A1917] border border-[#E9E4D8] dark:border-[#3A3733]">
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-[#211F1C] border border-[#E4DFD3] dark:border-[#3A3733] p-6 rounded-md space-y-3">
              <h3 className="text-sm font-serif text-slate-900 dark:text-white">Kesehatan Sistem</h3>
              <div className="space-y-2 text-xs">
                <div className="flex justify-between font-semibold"><span>Supabase DB</span><span className="text-[#3F7350]">Sehat</span></div>
                ...
                <div className="flex justify-between font-semibold pt-2"><span>API Latency</span><span className="text-[#6B8874] font-mono">24ms</span></div>
              </div>
            </div>
            {/* Ubah blok Aksi Cepat */}
            <div className="bg-[#20302A] p-6 rounded-md text-white space-y-3">
        ...
        {/* 5. USER MANAGEMENT TABLE WITH SEARCH, FILTER & PAGINATION */}
        <div className="bg-white dark:bg-[#211F1C] border border-[#E4DFD3] dark:border-[#3A3733] rounded-md shadow-xl overflow-hidden  space-y-4">
          <div className="p-6 border-b border-[#E9E4D8] dark:border-[#3A3733] flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h3 className="text-base font-serif text-slate-900 dark:text-white">Manajemen Pengguna</h3>
              <p className="text-xs text-slate-400">Kelola akses, peran, dan aktivitas pengguna terdaftar</p>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-[#3A3733] text-xs font-medium">
                {loading ? (
                  <tr>
                    <td colSpan="6" className="text-center py-8 text-slate-400">Memuat data pengguna...</td>
                  </tr>
                ) : paginatedUsers.length === 0 ? (
                  <tr>
                    <td colSpan="6" className="text-center py-8 text-slate-400">Tidak ada pengguna ditemukan.</td>
                  </tr>
                ) : (
                      ...
                      <tr key={user.id} className="hover:bg-[#F5F3ED] dark:hover:bg-slate-800/40 transition">
                        ...
                        <td className="py-4 px-6">
                          <span className={`px-2.5 py-1 rounded-sm text-[10px] font-bold uppercase ${
                            user.role === 'admin' ? 'bg-[#3D5F49]/10 text-[#6B8874] border border-[#3D5F49]/20' : 'bg-slate-500/10 text-slate-400'
                          }`}>
                            {user.role || 'USER'}
                          </span>
                        </td>
                        <td className="py-4 px-6 text-center font-bold text-slate-800 dark:text-slate-200">{user.todo_count}</td>
                        <td className="py-4 px-6 text-center font-bold text-[#3F7350]">{user.completed_count}</td>
                        <td className="py-4 px-6 text-right">
                          {/* Sembunyikan tombol jika itu akun sendiri ATAU email yang dilindungi */}
